  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
        <div className="rounded-xl border bg-emerald-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600">A tiempo</p><p className="text-2xl font-extrabold text-emerald-700">{summaryQuery.data?.onTime ?? 0}</p></div>
        <div className="rounded-xl border bg-amber-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-amber-600">Tardanzas</p><p className="text-2xl font-extrabold text-amber-700">{summaryQuery.data?.late ?? 0}</p></div>
        <div className="rounded-xl border bg-rose-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-rose-600">Faltas</p><p className="text-2xl font-extrabold text-rose-700">{summaryQuery.data?.absent ?? 0}</p></div>
        <div className="rounded-xl border bg-blue-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-blue-600">Justificados</p><p className="text-2xl font-extrabold text-blue-700">{summaryQuery.data?.justified ?? 0}</p></div>
        <div className="rounded-xl border bg-violet-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-violet-600">Pend. salida</p><p className="text-2xl font-extrabold text-violet-700">{summaryQuery.data?.pendingCheckOut ?? 0}</p></div>
        <div className="rounded-xl border bg-slate-50 p-3 text-center"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Sin registro</p><p className="text-2xl font-extrabold text-slate-600">{summaryQuery.data?.noRecord ?? 0}</p></div>
      </div>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  {r.isAbsent ? <Badge variant="danger">Falta</Badge>
                  : r.isJustified ? <Badge variant="success">Justificado</Badge>
                  : r.lateMinutes > 0 ? <Badge variant="warning">Tardanza {r.lateMinutes} min</Badge>
