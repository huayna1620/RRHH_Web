...
function SidebarContent({ onLinkClick }: { onLinkClick?: () => void }): JSX.Element {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto scrollbar-subtle bg-[#0d1b2a]">
      {/* Brand header */}
      <div className="flex h-14 shrink-0 items-center gap-2.5 border-b border-white/10 px-4">
        <div className="flex size-7 items-center justify-center rounded-lg bg-gradient-to-br from-brand-400 to-brand-600 shadow-lg shadow-brand-600/40">
          <span className="text-[11px] font-black text-white">RH</span>
        </div>
        <div>
          <p className="text-[13px] font-extrabold leading-none tracking-tight text-white">SAE · RRHH</p>
          <p className="text-[10px] text-white/40 leading-none mt-0.5">Sistema de Gestión</p>
        </div>
      </div>

      {/* Navigation */}
      <nav className="flex-1 space-y-0.5 px-2 py-3 pb-8">
          ...
          return (
            <div key={group.label} className="mb-1">
              ...
              <button
                ...
                className="flex w-full items-center justify-between rounded-md px-2 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-white/30 hover:text-white/50 transition-colors"
              >
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        {({ isActive }) => (
                          <>
                            <span className={`flex size-6 shrink-0 items-center justify-center rounded-md transition-colors ${isActive ? "bg-brand-500/30 text-brand-300" : "text-white/40 group-hover:text-white/70"}`}>
...
export function BarraLateral({ mobileOpen, onClose }: BarraLateralProps): JSX.Element {
  return (
    <>
      {/* Desktop */}
      ...
      {/* Mobile drawer */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 transition-transform duration-300 lg:hidden ${
      ...
      >
        <div className="absolute right-3 top-3 z-10">
          <button
            onClick={onClose}
            className="flex size-7 items-center justify-center rounded-lg bg-white/10 text-white/70 hover:bg-white/20 hover:text-white"
          >
            <X className="size-4" />
          </button>
        </div>
